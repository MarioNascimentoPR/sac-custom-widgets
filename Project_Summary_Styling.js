...
(function () {
  ...
  /* ==========================================================================
     4 & 5. ENGINE DE GENERATIVIDADE INTEGRAL - HEADLESS / NO-UI DETACHED
     ========================================================================== */
  class EvoNarrativeEngine {
    ...
    analyze(cubeData, targetNode, currentYear, previousYear, tempoDimId, versaoDimId, itemFinanceiroDimId, contaContabilDimId, measId, fullSeries) {
      // 4. Estruturação Interna Complexa (Feature Tables Reais)
      const varianceTable = { month: {}, ytd: {} };
      const driverTable = [];
      const outlierTable = [];
      const rankingTable = [];

      // A. População e Extração de Features Globais (Mês / YTD)
      ...
      // B. Processamento Multididimensional por Linha (Item ➔ Conta)
      const aggregationMap = {};
      cubeData.forEach(row => {
        if (!tempoDimId || !itemFinanceiroDimId) return;
        const tObj = row[tempoDimId]; if (!tObj) return;
        
        const rowMonthNode = fullSeries.find(d => d.id === String(tObj.id));
        ...
        // Cláusulas de Barreira / Higienização Corporativa Estrita
        if (
        ...
        if (contaName.toUpperCase().includes("RATEIO") || contaName.toUpperCase().includes("LIQUIDA")) return;

        if (!aggregationMap[itemName]) {
          aggregationMap[itemName] = { itemName, realizado: 0, orcado: 0, contas: {} };
        }
        ...
        if (isBudget) { aggregationMap[itemName].orcado += rawValue; } 
        else { aggregationMap[itemName].realizado += rawValue; }

        if (!aggregationMap[itemName].contas[contaName]) {
          aggregationMap[itemName].contas[contaName] = { realizado: 0, orcado: 0 };
        }
        if (isBudget) { aggregationMap[itemName].contas[contaName].orcado += rawValue; } 
        else { aggregationMap[itemName].contas[contaName].realizado += rawValue; }
      });

      // C. Estruturação das Tabelas de Drivers, Outliers e Modelagem de Score
      Object.keys(aggregationMap).forEach(name => {
        const item = aggregationMap[name];
        ...
        Object.keys(item.contas).forEach(cName => {
          const cDiff = item.contas[cName].realizado - item.contas[cName].orcado;
          if (Math.abs(cDiff) > maxContaImpact) {
            maxContaImpact = Math.abs(cDiff);
        ...
        // 3. Mecanismo de Inteligência Analítica (Insight Scoring / Noise Filter)
        // Atribui pontuação de severidade absoluta e elimina ruídos sem variação real
        if (Math.abs(desvioNominal) > 1000) { 
          outlierTable.push(featureRow);
        }
      });

      // D. Insight Scoring Table (Ordenação Estrita por Magnitude de Impacto Financeiro)
      rankingTable.push(...outlierTable);
      rankingTable.sort((a, b) => Math.abs(b.desvio) - Math.abs(a.desvio));

      // Limitador de Ruído Ativo: Corta a cauda longa de variações irrelevantes, mantendo os top drivers
      const filteredOutliers = rankingTable.slice(0, 5);

      return { varianceTable, driverTable, outlierTable: filteredOutliers, rankingTable };
    }
  ...
  class EvoSummaryWidget extends HTMLElement {
    constructor() {
      ...
      // Inicialização da Engine Acoplada Decoupled / Headless
    ...
    renderChart() {
      ...
      try {
        ...
        this._sortedMonths = Object.values(timelineMap);
        if (this._sortedMonths.length === 0) return;

        const fullSeriesData = [];
        let defaultActualIndex = -1;

        this._sortedMonths.forEach((m) => {
          ...
          fullSeriesData.push({ 
            id: m.id, label: alignedLabel, value: m.realizado, type: m.isCurrentMonth ? "actual" : "historical", originalNode: m, yearValue: parsedYear, monthNum: targetMonthIndex, rawRow: m.rawRow 
          });
        });
        ...
        // 2 & 5. RENDERIZAÇÃO ACOPLADA VIA EXECUÇÃO DO CONTRATO HEADLESS SEM INTERFERÊNCIA VISUAL
        this._renderDoubleFinancePanel(visibleSeriesData, fullSeriesData, actualIndex);
    ...
    /* ==========================================================================
       ETAPA DE RENDERING (UI LAYER) - CONSUMO DOS ARTIFACTS DA ENGINE PURE JS
       ========================================================================== */
    _renderDoubleFinancePanel(visibleSeriesData, fullSeriesData, actualIndex) {
      ...
      // Chamada Pura da Engine Sem UI Decoupled (Invocação Headless)
      ...
      // Re-população dos nós estruturais atômicos do painel numérico usando a varianceTable extraída
      const mFeatures = analysis.varianceTable.month;
      const yFeatures = analysis.varianceTable.ytd;

      this._valDiffRow.textContent = (mFeatures.diffNominal >= 0 ? "+" : "") + formatM(mFeatures.diffNominal);
      this._valPctRow.textContent = formatPercent(mFeatures.pctVar, mFeatures.isSaving || mFeatures.diffNominal <= 0);
      this._valPctRow.className = "status-badge-finance " + (mFeatures.diffNominal <= 0 ? "success" : "warning");
      this._valPctConsumptionRow.textContent = mFeatures.consumption.toFixed(2) + "%";

      this._ytdDiffRow.textContent = (yFeatures.diffNominal >= 0 ? "+" : "") + formatM(yFeatures.diffNominal);
      this._ytdDiffPctBadge.textContent = formatPercent(yFeatures.pctVar, yFeatures.diffNominal <= 0);
      this._ytdDiffPctBadge.className = "status-badge-finance " + (yFeatures.diffNominal <= 0 ? "success" : "warning");
      this._ytdPctRow.textContent = yFeatures.consumption.toFixed(2) + "%";

      // Sincronização e Repaint da mini esteira YTD lateral
      const maxYTD = Math.max(yFeatures.previous, yFeatures.actual, yFeatures.budget) * 1.10 || 1;
      this._miniBarPrev.style.height = `${(yFeatures.previous / maxYTD) * 100}%`;
      this._miniBarAct.style.height = `${(yFeatures.actual / maxYTD) * 100}%`;
      this._miniBarBud.style.height = `${(yFeatures.budget / maxYTD) * 100}%`;

      this._miniLblPrev.textContent = formatM(yFeatures.previous);
      this._miniLblAct.textContent = formatM(yFeatures.actual);
      this._miniLblBud.textContent = formatM(yFeatures.budget);
      ...
      this._ytdSeriesMock = [{ value: yFeatures.previous, type: "historical" }, { value: yFeatures.actual, type: "actual" }, { value: yFeatures.budget, type: "budget" }];

      if (this._highlightCardArea) {
        this._highlightCardArea.style.borderLeft = yFeatures.diffNominal <= 0 ? "4px solid #2E7D32" : "4px solid #D32F2F";
      }

      // Renderização do Novo Banner de contexto de período compactado no painel esquerdo
      if (this._periodSummaryBanner) {
        this._periodSummaryBanner.style.display = "block";
        this._periodSummaryBanner.className = yFeatures.diffNominal <= 0 ? "period-summary-banner summary-saving" : "period-summary-banner summary-desvio";
        ...
        spanYtdStatus.textContent = yFeatures.diffNominal <= 0 ? "abaixo do teto orçamentário (eficiência) " : "acima da meta prevista (atenção) ";
        ...
        spanYtdDelta.textContent = `R$ ${Math.abs(yFeatures.diffNominal/1000000).toFixed(2)}M (${yFeatures.diffNominal >= 0 ? "+" : ""}${yFeatures.pctVar.toFixed(1)}%)`;
        this._periodSummaryBanner.appendChild(spanYtdDelta);
        this._periodSummaryBanner.appendChild(document.createTextNode(`, absorvendo ${yFeatures.consumption.toFixed(1)}% do orçamento total.`));
      }

      // Limpeza e população iterativa do Bullet de Destaques Dinâmicos por Item (UI Rendering loop)
      this._hlUl.textContent = "";

      const monthStatusText = mFeatures.diffNominal <= 0 ? "economia de custos" : "estouro orçamentário";
      const semanticColorMonth = mFeatures.diffNominal <= 0 ? "#2E7D32" : "#D32F2F";
      ...
      liMonth.appendChild(s1); liMonth.appendChild(document.createTextNode("Fechamento com ")); liMonth.appendChild(statusSpan1); liMonth.appendChild(document.createTextNode(` de R$ ${Math.abs(mFeatures.diffNominal/1000000).toFixed(2)}M.`));
      ...
      const statusSpan2 = document.createElement("span"); statusSpan2.textContent = `${mFeatures.consumption.toFixed(1)}%`; statusSpan2.style.color = semanticColorCons; statusSpan2.style.fontWeight = "700";
      ...
      // Loop Dinâmico sobre a Outlier Table gerada e priorizada por severidade na Engine
