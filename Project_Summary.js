/* ==========================================================================
   EVOSTREAM SUMMARY WIDGET - VERSÃO DE ALTA PERFORMANCE (UNIFICADO)
   ========================================================================== */
...
(function () {
  ...
  class EvoSummaryWidget extends HTMLElement {
    constructor() {
      ...
      // Defesa de Escopo: Event Listener encapsulado para mitigar Memory Leaks globais
    ...
    renderChart() {
      ...
      try {
        ...
        // Reconciliação Estrutural de Performance (Eliminação completa de Churn de DOM)
        this._reconcileBarsAndLabels(visibleSeriesData, maxVal);

        // Desacoplamento Geométrico via RAF Consecutivo (Sanidade contra Layout Thrashing)
        requestAnimationFrame(() => {
          ...
          this._drawUnifiedFlatConnections(this._svgYtdOverlay, this._ytdChartArea, ".ytd-col .bar-element", this._ytdSeriesMock, 1, "ytd");
        });
    ...
    _renderDoubleFinancePanel(fullSeriesData, actualIndex, budgetVal) {
      ...
      // Proteção Absoluta contra Injeção de Dados Dinâmicos via innerHTML (Mitigação Estrita de XSS)
...
/* ==========================================================================
   PAINEL DE CONFIGURAÇÃO LATERAL (BUILDER PANEL INTEGRADO)
   ========================================================================== */
